...
import { customerApi, CustomerAgingDto, CustomerLedgerDto, PendingCustomerRowDto, PendingSummaryDto } from '../../api/customers';
...
export const ReceivablesPage = () => {
  ...
  const [pendingSummary, setPendingSummary] = useState<PendingSummaryDto | null>(null);
  const [aging, setAging] = useState<CustomerAgingDto | null>(null);

  const [ledgerLoading, setLedgerLoading] = useState(false);
  const [activeLedger, setActiveLedger] = useState<CustomerLedgerDto | null>(null);
  ...
  const loadOverview = async () => {
    ...
    try {
      const [pendingData, agingData] = await Promise.all([
        customerApi.pendingSummary(),
        customerApi.aging(asOfDate),
      ]);

      setPendingSummary(pendingData);
      setAging(agingData);
    } catch {
      toast.error('Failed to load receivable tracking data');
    } finally {
  ...
  const openLedger = async (customerId: number) => {
    ...
    try {
      const ledger = await customerApi.ledger(customerId);
  ...
  const canViewReceivables = hasPermission('customer.view') || hasPermission('sales.view') || hasPermission('invoices.view');
    ...
    navigate('/accounts', {
      state: {
        openPaymentWith: {
          ...
          customer_id: Number(activeLedger.customer.id),
          customer_name: activeLedger.customer.name,
  ...
  const handlePayNow = (order: CustomerLedgerDto['orders'][number]) => {
  ...
  const pendingColumns = [
    {
      header: 'Customer',
      accessorKey: 'customer_name' as keyof PendingCustomerRowDto,
      sortable: true,
    },
    {
      header: 'Open Invoices',
      accessorKey: 'open_orders' as keyof PendingCustomerRowDto,
      sortable: true,
    },
    {
      header: 'Total Due',
      accessorKey: 'total_due' as keyof PendingCustomerRowDto,
      sortable: true,
      cell: (row: PendingCustomerRowDto) => (
    ...
    },
    {
      ...
      cell: (row: PendingCustomerRowDto) => (
        <button
          type="button"
          onClick={() => void openLedger(row.customer_id)}
    ...
    <div className="space-y-4">
      <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
        <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div>
            <h2 className="text-base font-bold text-gray-900">Receivables Tracking</h2>
            <p className="text-xs text-gray-500">Track pending customer invoices, payment allocations, and aging buckets.</p>
          </div>
          <div className="flex items-end gap-2">
            <div>
              <label htmlFor="aging-as-of-date" className="mb-1 block text-[11px] font-semibold uppercase text-gray-500">Aging As Of</label>
              <input
                id="aging-as-of-date"
                type="date"
                value={asOfDate}
        ...
        <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-4">
          <div className="rounded-md border border-gray-200 bg-gray-50 p-3">
            <div className="text-[11px] uppercase text-gray-500">Customers with Due</div>
            <div className="mt-1 text-lg font-bold text-gray-900">{pendingSummary?.total_customers ?? 0}</div>
          </div>
          <div className="rounded-md border border-red-200 bg-red-50 p-3">
            <div className="text-[11px] uppercase text-red-600">Total Receivable</div>
      ...
      <DenseTable
        ...
        title="Customer Pending Summary"
        canSearch
        canExport
        canAdd={false}
      />
      ...
      <Modal
        ...
        title={activeLedger ? `Ledger - ${activeLedger.customer.name}` : 'Ledger'}
        size="xl"
      >
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
              ...
              <div className="rounded-md border border-blue-200 bg-blue-50 p-2">
                <div className="text-[11px] uppercase text-blue-600">Unallocated Credit</div>
                <div className="text-sm font-semibold text-blue-700">{formatAmount(activeLedger.summary.unallocated_credit)}</div>
              </div>
            </div>
